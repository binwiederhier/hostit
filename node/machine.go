...
func NewSystemServices(runner run.Runner, nodeID, bindAddr string, allowFrom []string, localProxyUID int, outboundAllowCIDRs []string) *Services {
	return &Services{
		...
		SSH:       ssh.New(),
		Firewall:  nftables.New(FirewallTable(nodeID), bindAddr, allowFrom, localProxyUID, outboundAllowCIDRs),
		Runner:    runner,
	}
}
