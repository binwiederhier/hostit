...
func (m *Machine) apply(a *store.App, conf *app.Config, allowReload bool) (string, error) {
	name := a.Name
	...
	if err := m.workspace.EnsureAppSubvolume(a, budgetGroup(m.uidFor(a.Port))); err != nil {
		return "", err
	}
...
func (m *Machine) LoadAppConfig(name string) (*app.Config, error) {
	root, err := m.homefs.OpenRoot(m.AppFiles(name))
	if err != nil {
		return nil, err
	}
