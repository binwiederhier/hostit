# bash completion for hostit-node, answered by the CLI itself: the binary prints the
# candidates for the current word when called with --generate-bash-completion.
_hostit_node_completion() {
...
complete -o bashdefault -o default -o nospace -F _hostit_node_completion hostit-node
