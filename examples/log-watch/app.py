...
Two hostit ideas in one small app:
  ...
  * It asks a MODEL a question with no API key of its own, over its own unix
    socket (see judge()).
  * It sends the notification through a GRANTED ntfy credential, which it reads
    from the same socket at the moment it needs it (see push()).
...
def socket_json(method, path, body=None):
    ...
    if resp.status != 200:
        raise RuntimeError(f"{method} {path}: HTTP {resp.status}: {raw[:200]}")
...
def judge(text):
    ...
    """
    return socket_json("POST", "/api/container/assistant", {
        "system": RULE,
        "prompt": f"Log lines:\n\n{text[-6000:]}",
        "max_tokens": 100,
    })["text"].strip()
...
def push(title, message):
    ...
    tok = socket_json("GET", f"/api/container/connections/{NTFY}/token")
