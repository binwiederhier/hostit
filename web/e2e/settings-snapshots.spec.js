...
test("snapshot settings round-trip through hostit.yml", async ({ page }, workerInfo) => {
  ...
  try {
    ...
    await section.getByLabel("Snapshot interval").selectOption("6h");
    await section.getByText("Pre/post snapshot commands").click();
    ...
    await expect(again.getByLabel("Snapshot interval")).toHaveValue("6h");
    await again.getByText("Pre/post snapshot commands").click();
    await expect(again.getByLabel("Before (pre)")).toHaveValue("echo pre-hook");

    // (The interval is a dropdown, so a nonsense value is no longer reachable
    // from here. The server still refuses one -- see the Go handler test.)
  } finally {
