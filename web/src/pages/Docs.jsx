...
const IntroPage = () => (
  <>
    <h2>Introduction</h2>
    <p>
      hostit gives every app its own container, a subdomain with HTTPS, SSH access, and an API an AI assistant can drive.
    ...
    <p>
      ...
      <span className="mono">https://&lt;name&gt;.{baseDomain}</span>. Nothing is broken while you build: there is always
      something at that URL.
    </p>
    <p>
      Every app gets its own container. You are root inside it, other apps are invisible, and an app cannot reach another
      app's files, processes or ports. Each container comes with the common runtimes already installed: Python 3 (venv and
      pip), the Go toolchain, Node.js with npm, PHP, and sqlite3, plus git, curl, rsync, htop, vim and nano. Anything else
      installs with <span className="mono">apt-get</span>.
    </p>
    <h3>The workspace</h3>
    <p>
      The app's page is a workspace: a built-in AI assistant that builds and deploys the app for you, a live preview beside
      it, and a browser terminal -- so you can do everything here, or over SSH and the API, or with your own agent.
    </p>
    ...
    <p>
      ...
      <strong>Settings</strong> (custom domains, rename, the API token and SSH keys). Each is covered in its own page in this
      guide.
    </p>
  </>
);
...
const AppsPage = () => (
  <>
    ...
    <p>
      One file decides how an app runs: <span className="mono">hostit.yml</span> in the app's home. Pick a mode, then apply it
      with <span className="mono">hostit deploy</span> over SSH or <span className="mono">POST /api/apps/&lt;app&gt;/deploy</span>.
      Keys hostit does not know are an error, so a typo is reported rather than ignored.
    </p>
    <h3>mode: static</h3>
    <Snippet text={`description: What this app is, in one line\nmode: static`} />
    <p>
      hostit serves <span className="mono">public/</span> itself. Nothing to run, nothing to install -- the directory is always
      <span className="mono"> public/</span>. This is the right mode for plain HTML and for the built output of any frontend.
    </p>
    <h3>mode: app</h3>
    <Snippet
...
env:
  DEBUG: "true"`}
    ...
    <p>
      ...
      starting a broken app. The command must listen on <span className="mono">0.0.0.0:$PORT</span>;{" "}
      <span className="mono">$PORT</span> is provided, and hostit restarts the command if it exits.
    </p>
    <p>
      ...
      bounded (one minute by default, five at most); once the command works, move it into <span className="mono">prepare:</span>.
    </p>
    ...
    <p>
      ...
      builds on the machine that runs it, so there is no cross-compiling and you need no toolchain of your own -- which matters
      if you are driving all of this from a chat window. And the app stays editable: the next session, yours or an assistant's,
      can read the source and change it.
    </p>
    <Snippet text={`prepare: cd src && go build -o ../bin/myapp .\nrun: ./bin/myapp`} />
    <h3>snapshot hooks (optional)</h3>
    <p>
      If the app has a database, add hooks so a snapshot captures a consistent copy: <span className="mono">pre</span> runs
      before the snapshot, <span className="mono">post</span> after. If <span className="mono">pre</span> fails, the snapshot is
      skipped rather than saving a torn state.
    </p>
    <Snippet
      text={`snapshot:
...
const AssistantPage = () => (
  <>
    ...
    <p>
      This is what hostit is for. Every app has an assistant that can build and change it for you, and the same app works with
      your own agent (like Claude Code) over the API.
    </p>
    <h3>The built-in chat</h3>
    <p>
      Open the app's page and use the <strong>Assistant</strong> tab. Ask in plain English ("add a leaderboard", "make the
      header dark"); it reads and writes the app's files, runs commands in the container, and deploys, with a live preview
      beside the chat. Its only tools are that one app's own operations, so it can never touch another app or the host.
    </p>
    <p className="hint">
      The built-in assistant needs the server to be configured with an AI key. If it is off, use SSH or your own agent (below).
    </p>
    <h3>Using it with Claude Code (or any agent)</h3>
    <p>
      Open your app's page and copy the prompt at the top. Paste it into Claude Code (or any assistant that can make HTTP
      requests) and it has everything it needs: the app's URL, a token scoped to that one app, and the address of an endpoint
      that explains the rest of the API to it.
    </p>
    <p>
      The prompt changes with the app. While the app is still a placeholder it asks the assistant to build something; once the
      app describes itself in <span className="mono">hostit.yml</span>, the prompt says the app already exists and asks the
      assistant to pick up where the last session left off.
    </p>
    <p>
...
const FilesPage = () => (
  <>
    ...
    <table className="docs-table">
      <tbody>
        ...
        <tr>
          ...
          <td>Binaries and scripts the app runs, e.g. <span className="mono">run: ./bin/myapp</span>.</td>
        </tr>
        <tr>
          <td className="mono">log/</td>
          <td>The app's output, written by hostit. <span className="mono">hostit logs</span> reads it.</td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td>The app's own documentation: how it works and why. Read it before changing the app, update it after.</td>
        </tr>
        <tr>
    ...
    <p>Directories appear as you write into them. The Files tab edits any of them; "Save &amp; deploy" writes the file and redeploys.</p>
    <p className="hint">
      If your app serves files itself (rather than <span className="mono">mode: static</span>), point it at{" "}
...
const SSHPage = () => (
  <>
    ...
    <p>
      The <strong>Terminal</strong> tab drops you straight inside the app's container in the browser. For your own machine, add
      an SSH key to your <a href="/profile">profile</a> first -- it applies to every app you own, present and future. Without a
      key, SSH cannot work and the app page says so.
    </p>
    <Snippet
...
const SnapshotsPage = () => (
  <>
    ...
    <p className="hint">
      Retention keeps history useful without growing forever (a grandfather-father-son policy). Snapshots live on the same disk
      and are not a substitute for an off-box backup.
    </p>
  </>
);
...
const DomainsPage = () => (
  <>
    ...
    <p>
      Serve an app on your own hostname on top of its subdomain. Add it from the app page's Actions menu (or{" "}
      <span className="mono">hostit apps domain add</span>); hostit shows the two DNS records to create and obtains the
      certificate over DNS-01, which works even when the server is not publicly reachable. The app is then reachable at both
      its <span className="mono">&lt;name&gt;.{baseDomain}</span> subdomain and your domain.
    </p>
    <h3>Renaming</h3>
    <p>
      Change an app's name from the app page's Settings tab. The app keeps running and nothing moves, so its files, container
      and custom domains all follow it; only the <span className="mono">&lt;name&gt;.{baseDomain}</span> subdomain and the SSH
      login change. Links to the old subdomain stop working.
    </p>
  </>
);
    ...
    <div className="table-wrap">
      <table className="docs-table">
        <tbody>
          ...
          <Endpoint method="POST" path="/api/apps/{app}/run" what={`Run one command in the container: {"command": "cd src && go build ./..."}`} />
...
const InstallPage = () => (
  <>
    ...
    <h3>1. DNS</h3>
    <p>
      Apps are served at <span className="mono">&lt;app&gt;.&lt;base-domain&gt;</span>, so point a wildcard and the bare name at
      the host:
    </p>
    <Snippet
...
apps.example.com.    A  <ip-of-this-host>   ; SSH and the bare domain`}
    ...
    <p>
      Snapshots, rollback, fork and hard disk quotas need <span className="mono">apps-dir</span> (default{" "}
      <span className="mono">/var/lib/hostit/apps</span>) on a btrfs filesystem, and hostit refuses to start without it. Put it
      on a btrfs mount before starting; a loopback file works for a small host, and the example Ansible role does this for you.
    </p>
    ...
    <Snippet text={`base-domain: apps.example.com\nadmin-token: $(openssl rand -hex 24)`} />
    ...
    <p>
      ...
      Encrypt. To get the web dashboard and Google login, and (recommended) wildcard TLS, see <strong>Configuration</strong>.
    </p>
  </>
);
...
const ConfigPage = () => (
  <>
    <h2>Configuration</h2>
    <p>
      Everything lives in <span className="mono">/etc/hostit/server.yml</span>. Only <span className="mono">base-domain</span>{" "}
      and <span className="mono">admin-token</span> are required; the rest has sane defaults. Restart the service after editing
      (<span className="mono">systemctl restart hostit</span>).
    </p>
    <div className="table-wrap">
      <table className="docs-table">
        <tbody>
          ...
          <tr>
            ...
            <td>Registry DB + certs, and app home directories. Default <span className="mono">/var/lib/hostit</span> and{" "}
              <span className="mono">/var/lib/hostit/apps</span> (btrfs).</td>
          </tr>
          <tr>
            <td className="mono">listen-http / -https / -api</td>
            <td>Listener addresses. <span className="mono">listen-api</span> is an optional extra plain-HTTP admin listener, e.g. <span className="mono">127.0.0.1:2900</span>.</td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td>Enables the built-in assistant on the metered Anthropic API. See Users and administration.</td>
          </tr>
          <tr>
...
const AdminPage = () => (
  <>
    ...
    <p>
      ...
      <span className="mono">admin-emails</span>, or whom another admin has promoted.
    </p>
    <h3>Approving people</h3>
    <p>
      A new Google sign-in lands as <em>pending</em> and cannot create apps until an admin approves it, so an open Google login
      does not mean open signups. Two ways to skip the wait: list a whole email domain under allowed domains (anyone at{" "}
      <span className="mono">@yourco.com</span> is approved automatically), or invite a specific person, which pre-approves
    ...
    <p>
      The in-browser chat is off until the server has an AI key. Set <span className="mono">anthropic-api-key</span> (pay per
      token) for the API models, and/or a Claude Pro/Max subscription token (<span className="mono">claude-code-oauth-token</span>{" "}
      from <span className="mono">claude setup-token</span>) to additionally offer Claude.ai, run in a locked-down podman
      sandbox. Each credential's presence is the whole switch. On the Admin page you choose which models people may pick, the
      default, and who may use the assistant at all.
    </p>
    <h3>Backups</h3>
    <p>
      All durable state is two things: the SQLite registry under <span className="mono">data-dir</span> (accounts, apps, tokens,
      domains, snapshot metadata) and the app files under <span className="mono">apps-dir</span>. Back up both. hostit already
      snapshots each app hourly, but those snapshots live on the same disk and are not a substitute for an off-box backup.
    </p>
    <Snippet text={`sqlite3 /var/lib/hostit/hostit.db ".backup /backup/hostit.db"   # hot registry copy`} />
    <p className="hint">
      hostit runs as root because it creates Unix users and drives podman, systemd, nftables and btrfs. Keep the host locked
      down; app containers are isolated, but the daemon is the trusted control plane.
    </p>
  </>
);
...
const TroubleshootingPage = () => (
  <>
    ...
    <Snippet
      text={`systemctl status hostit
journalctl -u hostit -f              # follow the daemon log
journalctl -u hostit -n 200 --no-pager   # recent history (preflight errors land here)`}
    />
    <p>
      On startup hostit preflights root, the required binaries (podman, nft, btrfs) and that <span className="mono">apps-dir</span>{" "}
      is btrfs. A failed preflight is one clear line at the top of the log.
    </p>

    <h3>Map an app name to its id</h3>
    <p>The registry knows both. Query it read-only:</p>
    <Snippet text={`sqlite3 /var/lib/hostit/hostit.db "select id, name, port from apps;"`} />

    <h3>The app's container (podman)</h3>
    <p>Containers are named <span className="mono">hostit-app-&lt;id&gt;</span>.</p>
    <Snippet
...
podman stats --no-stream                      # live CPU/memory per container`}
    ...
    <p>Each app runs as a template unit instance <span className="mono">hostit-app@&lt;id&gt;.service</span>.</p>
    <Snippet
...
tail -f /var/lib/hostit/apps/<id>/log/app.log # the run: command's output`}
    ...
    <p className="hint">
      ...
      a red "App crashed"). The reason is a <span className="mono">[hostit]</span> line at the end of{" "}
      <span className="mono">app.log</span>. Fix the app, then <span className="mono">hostit deploy</span> or{" "}
...
btrfs qgroup show -re /var/lib/hostit/apps         # per-app disk usage and quota`}
    ...
    <p>hostit keeps one table, <span className="mono">inet hostit</span>, that pins each app's loopback port to its owner's uid.</p>
    <Snippet text={`nft list table inet hostit`} />

    <h3>The registry (SQLite)</h3>
    <p>All non-file state is one SQLite database. Inspect it read-only while the daemon runs:</p>
    <Snippet
  ...
  select email, role, status from users;`}
    />
    <p className="hint">
      TLS certificates are under <span className="mono">/var/lib/hostit/certs</span>; the app-side CLI socket is{" "}
      <span className="mono">/run/hostit/hostit.sock</span>. When in doubt, <span className="mono">systemctl restart hostit</span>{" "}
      is safe: running app containers keep serving, and the daemon reconciles their state on startup.
    </p>
  </>
);
...
const guides = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: "troubleshooting", title: "Troubleshooting", render: TroubleshootingPage },
    ],
  },
];

const Docs = () => {
  const currentKey = window.location.pathname.startsWith("/docs/admin") ? "admin" : "user";
  const current = useMemo(() => guides.find((g) => g.key === currentKey), [currentKey]);
  const [activeId, setActiveId] = useState(() => window.location.hash.slice(1) || current.items[0].id);
  ...
  useEffect(() => {
    const sections = current.items.map((it) => document.getElementById(it.id)).filter(Boolean);
