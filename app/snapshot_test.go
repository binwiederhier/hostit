...
func keptIDs(snaps []Snapshot, p RetentionPolicy) []string {
	keep, _ := applyRetention(snaps, p)
...
func TestDefaultRetentionValues(t *testing.T) {
	t.Parallel()
	assert.Equal(t, RetentionPolicy{Last: 50, Daily: 7, Weekly: 4, Monthly: 3}, defaultRetention)
}

func TestRetentionEmpty(t *testing.T) {
	t.Parallel()
	keep, prune := applyRetention(nil, defaultRetention)
	assert.Empty(t, keep)
...
func TestRetentionKeepsNewestPerDay(t *testing.T) {
	...
	assert.Equal(t, []string{"evening"}, keptIDs(snaps, RetentionPolicy{Daily: 1}))
}
...
func TestRetentionDailyBeyondLast(t *testing.T) {
	...
	assert.Equal(t,
		...
		keptIDs(snaps, RetentionPolicy{Last: 2, Daily: 5}))
}
...
func TestRetentionMonthlyExtendsPastLast(t *testing.T) {
	...
	keep, prune := applyRetention(snaps, RetentionPolicy{Last: 1, Monthly: 3})
...
func TestRetentionZeroPolicyPrunesAllAuto(t *testing.T) {
	...
	keep, prune := applyRetention(snaps, RetentionPolicy{})
	assert.Empty(t, keep)
...
func TestRetentionWeekBucketsAcrossYearBoundary(t *testing.T) {
	...
	assert.Equal(t, []string{"dec", "jan"}, keptIDs(snaps, RetentionPolicy{Weekly: 2}))
}
...
func TestRetentionMonthlyKeepsNewestPerMonth(t *testing.T) {
	...
	assert.Equal(t, []string{"aug-late", "jul", "jun"}, keptIDs(snaps, RetentionPolicy{Monthly: 3}))
}
