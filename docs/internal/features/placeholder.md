...
The placeholder is not a special server mode or a running process. A new app is
scaffolded as a `mode: static` app whose `public/index.html` is the placeholder page,
so hostit serves that file directly.
...
Making the placeholder a plain `public/index.html` in the scaffold (rather than a
...
sequenceDiagram
    ...
    User->>API: create app
    API->>Mgr: CreateApp
    Mgr->>Mgr: WriteScaffold (hostit.yml: mode: static, public/index.html)
...
- When the owner (or an agent) edits `public/index.html`, or writes their own
  ...
  Because `WriteScaffold` never overwrites existing files, an app that already has
  content keeps it.

## Technical details

- **The page:** `app/scaffold/public/index.html`, embedded via
  `//go:embed scaffold/public/index.html` into `scaffoldPublicIndex`
  (`app/scaffold.go`). It is a self-contained HTML document with inline CSS
...
- **The config:** `app/scaffold/hostit.yml` is `mode: static`, with the description
  field and the `mode: app` alternative documented inline as comments.
- **Wired via the scaffold:** `app/scaffold.go:scaffoldFiles` returns the initial home
  files: `hostit.yml`, `public/index.html` (the placeholder), a templated `README.md`
  (`app/scaffold/readme.md`) that tells the reader the app is a stub, and `.hushlogin`.
- **Applied on create:** `app/service.go:create` calls
  `SystemOps.WriteScaffold(name, home, scaffoldFiles(...))` for a fresh (non-fork) app.
  `WriteScaffold` never overwrites existing files.
- **Served like any static app:** a `mode: static` app is served straight from
  ...
  earlier design ran one, and it was removed in favor of this static scaffold.

## Other notes

- **A fork does not get the placeholder.** Forking seeds the home from the source and
  skips the scaffold (`app/service.go:create`, the `forking` branch), so a fork serves
...
- **The placeholder is only the default, not a lock-in.** Editing `public/index.html`
  (or switching to `mode: app`) and deploying replaces it. The scaffold `README.md` and
...
- **Related:** [deploy.md](deploy.md) (the modes and the deploy path) and
  [apps-lifecycle.md](apps-lifecycle.md) (create, which installs the scaffold).
