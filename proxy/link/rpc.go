...
type certResp struct {
	Material *api.CertMaterial `json:"material,omitempty"`
	Err      string            `json:"err,omitempty"`
	ErrCode  string            `json:"err_code,omitempty"`
}

// RPCHandler serves a ProxyAgent over the duplex: control's calls arrive here.
func RPCHandler(agent api.ProxyAgent) http.Handler {
...
func NewRemoteProxy(client *http.Client) api.ProxyAgent {
...
func (r *remoteProxy) ApplyRoutes(table *api.Table) error {
...
func (r *remoteProxy) Heartbeat() *api.Heartbeat {
...
func CallbackHandler(sink api.ControlSink) http.Handler {
	mux := http.NewServeMux()
	mux.HandleFunc("POST "+certPath, func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			if errors.Is(err, api.ErrNoCert) {
...
func NewControlSink(client *http.Client) api.ControlSink {
...
func (c *controlSink) CertFor(sni string) (*api.CertMaterial, error) {
