...
// Control puts base security headers on app traffic; in any deployment with a
// proxy in front, control never sees that traffic. Without these here, every
// app loses HSTS and nosniff exactly where the proxy is doing its job.
func TestProxyServedResponsesCarryTheBaseSecurityHeaders(t *testing.T) {
	t.Parallel()
	appSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {}))
	defer appSrv.Close()
	p := New(&Config{ControlURL: "http://127.0.0.1:1", CacheDir: t.TempDir()})
