...
func TestReadFileMaxRefusesOversized(t *testing.T) {
	...
	b, err := m.testMachine().ReadFileMax("blog", "uploads/pic.bin", 2000)
	require.NoError(t, err)
	assert.Len(t, b, 1000)

	// Over the cap: refused, not truncated (guards the shared daemon's memory).
	_, err = m.testMachine().ReadFileMax("blog", "uploads/pic.bin", 500)
	assert.Error(t, err)
}
...
func TestFileExists(t *testing.T) {
	...
	assert.True(t, m.testMachine().FileExists("blog", "uploads/logo.png"))
	assert.False(t, m.testMachine().FileExists("blog", "uploads/missing.png"))
	assert.False(t, m.testMachine().FileExists("blog", "../../etc/passwd")) // traversal refused
}
...
func TestMoveFile(t *testing.T) {
	...
	require.NoError(t, m.testMachine().MoveFile("blog", "a.txt", "public/b.txt"))
	assert.False(t, m.testMachine().FileExists("blog", "a.txt"))
	b, err := m.testMachine().ReadFile("blog", "public/b.txt")
	require.NoError(t, err)
	assert.Equal(t, "hi", string(b))

	// Refuse to clobber an existing destination.
	writeAppFile(t, m, "blog", "c.txt", "x")
	assert.Error(t, m.testMachine().MoveFile("blog", "c.txt", "public/b.txt"))
	// Traversal out of the home is refused.
	assert.Error(t, m.testMachine().MoveFile("blog", "public/b.txt", "../escape.txt"))
}
...
func TestMakeDir(t *testing.T) {
	...
	require.NoError(t, m.testMachine().MakeDir("blog", "assets/img"))
	info, err := m.testMachine().StatFile("blog", "assets/img")
	require.NoError(t, err)
	assert.Equal(t, FileTypeDir, info.Type)

	// Refuse to create over an existing path, and refuse traversal.
	assert.Error(t, m.testMachine().MakeDir("blog", "assets/img"))
	assert.Error(t, m.testMachine().MakeDir("blog", "../escape"))
}
...
func TestStatFile(t *testing.T) {
	...
	info, err := m.testMachine().StatFile("blog", "notes.txt")
	require.NoError(t, err)
	...
	info, err = m.testMachine().StatFile("blog", "public/logo.png")
	require.NoError(t, err)
	assert.Equal(t, "image/png", info.Mime)

	// No extension + binary bytes: sniffed as a non-text type.
	info, err = m.testMachine().StatFile("blog", "data")
	require.NoError(t, err)
	...
	info, err = m.testMachine().StatFile("blog", "public")
	require.NoError(t, err)
	assert.Equal(t, FileTypeDir, info.Type)
	_, err = m.testMachine().StatFile("blog", "../../etc/passwd")
	assert.Error(t, err)
}
