...
func (s *Server) newAPIHandler() http.Handler {
	...
	route(mux, "GET", "/apps/{name}/viewers", s.requireActive(s.handleViewersList))
	route(mux, "POST", "/apps/{name}/viewers", s.requireActive(s.handleViewersAdd))
	route(mux, "DELETE", "/apps/{name}/viewers/{id}", s.requireActive(s.handleViewersRemove))
