...
func TestWriteAndReadFile(t *testing.T) {
	...
	require.NoError(t, m.testMachine().WriteFile("blog", "index.html", []byte("<h1>hi</h1>"), 0))
	b, err := m.testMachine().ReadFile("blog", "index.html")
	require.NoError(t, err)
	assert.Equal(t, "<h1>hi</h1>", string(b))
	// Nested paths are created on the way
	require.NoError(t, m.testMachine().WriteFile("blog", "static/css/site.css", []byte("body{}"), 0))
	b, err = m.testMachine().ReadFile("blog", "static/css/site.css")
	require.NoError(t, err)
...
func TestListFiles(t *testing.T) {
	...
	m, _, _ := newTestDeployManager(t)
	createTestApp(t, m, "blog")
	require.NoError(t, m.testMachine().WriteFile("blog", "index.html", []byte("x"), 0))
	require.NoError(t, m.testMachine().WriteFile("blog", "static/site.css", []byte("y"), 0))
	listing, err := m.testMachine().ListFiles("blog", "")
	require.NoError(t, err)
...
func TestDescriptionFromHostitYml(t *testing.T) {
	...
	require.NoError(t, m.testMachine().WriteFile("blog", "hostit.yml", []byte("description: Expense tracker for the finance team\nmode: static\n"), 0))
...
func TestDescriptionSurvivesAnUnfinishedConfig(t *testing.T) {
	...
	require.NoError(t, m.testMachine().WriteFile("blog", "hostit.yml", []byte("description: A half-written app\n"), 0))
	assert.Equal(t, "A half-written app", m.Description("blog"))
	// Two modes at once is invalid too, and still not a reason to forget
	require.NoError(t, m.testMachine().WriteFile("blog", "hostit.yml", []byte("description: Two modes\nmode: static\nrun: ./x\n"), 0))
	assert.Equal(t, "Two modes", m.Description("blog"))
	// Unparseable YAML has no description to offer
	require.NoError(t, m.testMachine().WriteFile("blog", "hostit.yml", []byte("description: [unclosed\n"), 0))
	assert.Empty(t, m.Description("blog"))
}
...
func TestDescriptionIgnoresAnAbsurdConfig(t *testing.T) {
	...
	require.NoError(t, m.testMachine().WriteFile("blog", "hostit.yml", huge, 0))
	assert.Empty(t, m.Description("blog"))
}
...
func TestSymlinksCannotEscapeTheAppHome(t *testing.T) {
	...
	b, err := m.testMachine().ReadFile("blog", "notes.txt")
	...
	// write through it
	require.NoError(t, m.testMachine().WriteFile("blog", "notes.txt", []byte("overwritten"), 0))
	...
	_, err = m.testMachine().ExtractTar("blog", &buf)
	...
	err = m.testMachine().DeleteFile("blog", "notes.txt")
	if err == nil {
...
func TestFilesRefuseAHomeSymlinkedOutOfTheSubvolume(t *testing.T) {
	...
	for _, plant := range []string{"home", "home/app"} {
		t.Run(plant, func(t *testing.T) {
			...
			require.Error(t, m.testMachine().WriteFile("blog", "x.txt", []byte("x"), 0), "writes through a planted %s link must be refused", plant)
			_, err := m.testMachine().ReadFile("blog", "hostit.yml")
			...
			_, err = m.testMachine().LoadAppConfig("blog")
			...
			state, startedAt := m.testMachine().AppProcessState("blog")
...
func TestProtectedPathsAreNotWritable(t *testing.T) {
	...
	for _, rel := range []string{
		...
		require.ErrorIs(t, m.testMachine().WriteFile("blog", rel, []byte("x"), 0), ErrInvalid, "write %q", rel)
		_, err := m.testMachine().ReadFile("blog", rel)
		require.ErrorIs(t, err, ErrInvalid, "read %q", rel)
		require.ErrorIs(t, m.testMachine().DeleteFile("blog", rel), ErrInvalid, "delete %q", rel)
	}
	// A dotfile of the app's own is still the app's business
	require.NoError(t, m.testMachine().WriteFile("blog", ".env", []byte("KEY=value"), 0))
}
