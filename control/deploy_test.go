...
func TestUpWorkspaceModeUnchangedOnlyReloadsAgent(t *testing.T) {
	...
	require.NoError(t, err)
	hash := workspace.ConfigHash(workspace.CreateArgs(conf, a, m.testMachine().AppSubvolume("blog"), m.config.SocketFile, workspace.HostBinFile, node.Version, 0, defaultCPUMilli, ids, ""))
