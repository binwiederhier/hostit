...
func TestMeasureDiskUsage(t *testing.T) {
	...
	usage, err := m.testMachine().MeasureDiskMB("blog")
...
func TestSetDiskLimitCapsTheBudgetGroupNotTheHome(t *testing.T) {
	...
	group := fmt.Sprintf("1/%d", workspace.UIDFor(a.Port))
	r.reset()
	m.testMachine().SetDiskLimit("blog", 512)
...
func TestSetDiskLimitZeroFallsBackToTheDefaultCap(t *testing.T) {
	...
	group := fmt.Sprintf("1/%d", workspace.UIDFor(a.Port))
	r.reset()
	m.testMachine().SetDiskLimit("blog", 0)
...
func TestRefreshDiskUsageRecordsUsageWithoutStopping(t *testing.T) {
	...
	m.testMachine().SetDiskLimit("blog", 1) // 1 MB limit, app uses ~3 MB
	runner.reset()
	require.NoError(t, m.testMachine().RefreshDiskUsage())
	a, err := m.App("blog")
	require.NoError(t, err)
	assert.GreaterOrEqual(t, a.DiskMB, 3)                                                   // usage recorded
	assert.NotContains(t, runner.ran(), "systemctl stop "+m.testMachine().UnitName("blog")) // never stopped
}
