...
func (s *Server) agentGuide(appName, description string) *apiAgentInfoResponse {
	...
	return &apiAgentInfoResponse{
		...
		Workflow: []string{
			...
			"Connected accounts: if the owner has connected Google, GitHub or an IMAP mailbox and granted it to this app, GET /api/self/connections over the app's unix socket lists them and GET /api/self/connections/{provider}/token returns a usable, short-lived credential. Ask for it per request and do not store it -- it expires. The app acts as its owner.",
		...
		Endpoints: []apiAgentEndpoint{
			...
			{Method: "GET", Path: "/api/self/connections", What: "Over the app's unix socket: the connections and credentials this app was granted, each with the slug its owner named it"},
			{Method: "GET", Path: "/api/self/connections/{slug}/token", What: "Over the app's unix socket: a usable, short-lived credential for one of them, e.g. curl --unix-socket $HOSTIT_SOCKET http://x/api/self/connections/work-cal/token"},
		},
		Notes: []string{
