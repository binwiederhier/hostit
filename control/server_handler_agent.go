...
func (s *Server) agentGuide(appName, description string) *apiAgentInfoResponse {
	...
	return &apiAgentInfoResponse{
		...
		Endpoints: []apiAgentEndpoint{
			...
			{Method: "GET", Path: "/v1/connections", What: "Over the app's unix socket: which connected accounts this app was granted"},
			{Method: "GET", Path: "/v1/connections/{provider}/token", What: "Over the app's unix socket: a short-lived credential for one of them, e.g. curl --unix-socket $HOSTIT_SOCKET http://x/v1/connections/google/token"},
		},
		Notes: []string{
