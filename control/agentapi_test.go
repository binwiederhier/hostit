...
func TestAgentTokenCannotTouchOtherApps(t *testing.T) {
	...
	s := newTestServer(t)
	token := newAppToken(t, s, "blog")
	createTestAppViaAgent(t, s, "other")
...
// createTestAppViaAgent creates an app owned by someone else
func createTestAppViaAgent(t *testing.T, s *Server, appName string) {
	t.Helper()
...
// fileInfoJSON mirrors the FileInfo shape returned by the stat endpoint.
