...
// SetNodeStats records what a node last reported about its own machine.
func (s *Store) SetNodeStats(name string, stats string) error {
	_, err := s.db.Exec(updateNodeStatsQuery, stats, name)
	return err
}
