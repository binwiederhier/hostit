...
func TestContainerCreateArgsWorkspaceMode(t *testing.T) {
	...
	assert.Contains(t, cmd, ImageTag())
	// The agent supervises the run command as PID 1
	assert.True(t, strings.HasSuffix(cmd, ImageTag()+" /usr/bin/hostit agent"), cmd)
}
...
func TestContainerConfigHashChanges(t *testing.T) {
	...
	hash1 := ConfigHash(CreateArgs(conf1, a, "/h", "/s", "/b", testVersion, 0, testIDs))
	hash2 := ConfigHash(CreateArgs(conf2, a, "/h", "/s", "/b", testVersion, 0, testIDs))
	hash3 := ConfigHash(CreateArgs(conf3, a, "/h", "/s", "/b", testVersion, 0, testIDs))
	hash4 := ConfigHash(CreateArgs(conf4, a, "/h", "/s", "/b", testVersion, 0, testIDs))
...
func TestConfigHashIgnoresTheHostname(t *testing.T) {
	...
	hashBefore := ConfigHash(CreateArgs(conf, before, "/h", "/s", "/b", testVersion, 0, testIDs))
	hashAfter := ConfigHash(CreateArgs(conf, after, "/h", "/s", "/b", testVersion, 0, testIDs))
...
func TestContainerUsesPinnedImage(t *testing.T) {
	...
	joined := strings.Join(CreateArgs(conf, a, "/home", "/run/hostit/x.sock", "/usr/bin/hostit", testVersion, 0, IDs{UID: 1001, GID: 1001}), " ")
	assert.Contains(t, joined, "localhost/hostit-workspace:pinned123")
	assert.NotContains(t, joined, ImageTag())

	// An app from before pinning (no tag) falls back to the current image.
	old := &store.App{Name: "old", Port: 10001}
	joinedOld := strings.Join(CreateArgs(conf, old, "/home", "/run/hostit/x.sock", "/usr/bin/hostit", testVersion, 0, IDs{UID: 1001, GID: 1001}), " ")
	assert.Contains(t, joinedOld, ImageTag())
}
...
func TestWithConfigLabelInsertsBeforeTheTrailer(t *testing.T) {
	...
	conf := &appctl.AppConfig{Mode: appctl.ModeApp, Run: "./server"}
	a := &store.App{ID: "appid123", Name: "blog", Port: 10000}
	args := CreateArgs(conf, a, "/h", "/run/hostit/hostit.sock", "/usr/bin/hostit", testVersion, 0, testIDs)
	labeled := WithConfigLabel(args, "abc123")
	// The label is a flag, so it must land before the trailing image+command;
	// injecting it just ahead of the trailer keeps podman's argv valid whatever
	// flags come before it.
	require.Len(t, labeled, len(args)+2)
	assert.Equal(t, "--label", labeled[len(labeled)-5])
	assert.Equal(t, "hostit.config=abc123", labeled[len(labeled)-4])
	assert.Equal(t, []string{ImageTag(), "/usr/bin/hostit", "agent"}, labeled[len(labeled)-3:])
	// The original args are untouched around the insertion point.
	assert.Equal(t, args[:len(args)-3], labeled[:len(labeled)-5])
}
